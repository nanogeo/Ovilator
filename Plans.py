            ...
            if await MacroLingBaneHydra.make_expansions(bot):
                if await MacroLingBaneHydra.make_drones(bot):
                    if await MacroLingBaneHydra.expand_tech(bot):
                        if await MacroLingBaneHydra.make_army(bot):
                            print("completed entire plan")
        if round((time.time() - T) * 1000) > 0:
            print("plan executed in " + str(round((time.time() - T) * 1000)) + "ms")
